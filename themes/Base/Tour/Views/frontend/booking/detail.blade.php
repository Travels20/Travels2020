    ...
    <div class="booking-review-content">
        ...
        <div class="review-section">
            <ul class="review-list">
                 ...
                 @php 
                    ...
                    @if(!empty($person_types))
                        <li>
                            ...
                            <div class="val">
                                @php $travellerInfo = []; @endphp
                                @foreach($person_types as $type)
                                    @php 
                                        $name = $type['name_' . $lang_local] ?? __($type['name']);
                                        $travellerInfo[] = $type['number'] . ' ' . $name;
                                    @endphp
                                @endforeach
                                {{ implode(', ', $travellerInfo) }}
                            </div>
                        </li>
                     @endif
                         <li>
                ...
                @if($service_translation->activities)
                    @php
                        // Decode the activities field into an array if it's a string
                        $activities = is_string($service_translation->activities) ? json_decode($service_translation->activities, true) : $service_translation->activities;
                    @endphp
                    <li>
                        <div class="label">{{ __('Activity:') }}</div>
                        <div class="val">
    ...
    <div class="card" >
     ...
     <!-- <div class="card">
        <div class="card-body">
            ...
            <div class="card-text">
                 @if($service_translation->title)
                 <i class="fa fa-map-marker"></i> {{$service_translation->title}}
                @endif

                @if($service_translation->content)
                <div class="description">
                    {!! $service_translation->content !!}
                </div>
                @endif
