<!DOCTYPE html>
<html>
<head>
    <title>Itinerary PDF</title>
    <style>
        body { font-family: Arial, sans-serif; font-size: 12pt; }
        h1 { text-align: center; font-size: 20pt; margin-bottom: 5px; font-weight: bold; color: rgb(11, 66, 105); }
        span { text-align: center; font-size: 14pt; margin-bottom: 10px; font-weight: bold; }
        table { width: 100%; border-collapse: collapse; margin-top: 15px; page-break-inside: avoid; }
        th, td { padding: 10px; text-align: left; border: 1px solid #000; }
        .header { background-color: #A4B5C1; color: #fff; font-weight: bold; }
        .theaders { background-color: rgb(237, 241, 242); color: #000; font-weight: bold; }
        .box { border: 1px solid #000; padding: 10px; margin-top: 20px; text-align: center;
            border-radius: 10px; background-color: #f5f5f5; page-break-inside: avoid; }
        p { margin: 10px 0; }
        h2 { page-break-inside: avoid; text-align: left; font-size: 16pt; margin-top: 5px; }
    </style>
</head>
<body>

    <a href="http://www.travels2020.com"><img src="{{ public_path('images/Header.jpg') }}" width="100%"></a>

    <table style="margin-top: 10px; border: none;">
        <tr>
            <td style="font-weight: bold; font-size: 15pt; text-align: left; border: none;">Hey, {{ $tour->username }}</td>
            <td style="text-align: right; border: none;"><strong>Generated On:</strong> {{ date('d-m-Y') }}</td>
        </tr>
    </table>

    <h1>{{ strtoupper($tour->tour_name) }} <br><span> ({{ $nights }} Nights / {{ $duration }} Days)</span></h1>

    @php
        $tourImage = $tour->tour_image;
        if (pathinfo($tourImage, PATHINFO_EXTENSION) == 'png') {
            $tourImage = str_replace('.png', '.jpg', $tourImage); // Optionally convert in controller
        }
    @endphp

    <img src="{{ public_path($tourImage) }}" width="100%" height="50%">

    <div class="box">
        <table>
            <tr class="theaders">
                <td><strong>TRIP ID:</strong></td>
                <td>{{ $tour->trip_id }}</td>
            </tr>
            <tr>
                <td><strong>NO. OF PASSENGER:</strong></td>
                <td>{{ $tour->adults }} adults, {{ $tour->children }} {{ $tour->children == 1 ? 'child' : 'children' }}</td>
            </tr>
            <tr class="theaders">
                <td><strong>DEPARTURE:</strong></td>
                <td>{{ \Carbon\Carbon::parse($tour->check_in)->format('d-m-Y') }}</td>
            </tr>
            <tr>
                <td><strong>ARRIVAL:</strong></td>
                <td>{{ \Carbon\Carbon::parse($tour->check_out)->format('d-m-Y') }}</td>
            </tr>
        </table>
    </div>

    <h2>Itinerary:</h2>
    @foreach($vacationSummaries as $index => $day)
        <div class="box">
            <table>
                <tr class="header">
                    <td><h3>Day {{ $index + 1 }}</h3></td>
                    <td>{{ $day->stay }}</td>
                    <td>{{ \Carbon\Carbon::parse($day->date)->format('d-m-Y') }}</td>
                </tr>
                <tr>
                    <td><img src="{{ public_path($day->image) }}" style="width: 100%; max-width: 200px; border-radius: 5px;"></td>
                    <td colspan="2">{!! $day->itinerary_content !!}</td>
                </tr>
            </table>
        </div>
    @endforeach

    @if(!empty($tour->cost))
        <div><h2>LAND PACKAGE COST:</h2>{!! $tour->cost !!}</div>
    @endif

    @if(!empty($tour->hotel))
        <h2>Hotel Details:</h2>{!! $tour->hotel !!}
    @endif

    @if(!empty($tour->flight))
        <h2>Flight Details:</h2>{!! $tour->flight !!}
        @if(!empty($tour->ftimage))
            <img src="{{ public_path($tour->ftimage) }}" width="100%" height="auto"><br><br>
        @endif
    @endif

    @if(!empty($tour->inclusion))
        <h2>Inclusion:</h2>{!! $tour->inclusion !!}
    @endif

    @if(!empty($tour->exclusion))
        <h2>Exclusion:</h2>{!! $tour->exclusion !!}
    @endif

    @if(!empty($tour->notes))
        <h2>Important Notes:</h2>{!! $tour->notes !!}
    @endif

    <div class="box">
        <table style="border: none;">
            <tr>
                <td style="border: none;"><img src="{{ public_path('images/pothy.jpg') }}" width="100" height="100" style="border-radius: 50%;"></td>
                <td style="text-align: center; border: none;">
                    <p>Your trip is planned by travels2020<br>Anbu Pothy<br><span>1500+ trips planned</span></p>
                    <p>For any queries or requests?</p>
                </td>
                <td style="text-align: right; border: none;">
                    <p>Say Hello<br>
                    <a href="https://wa.me/919445552020?text={{ urlencode('Hello, I would like to know more about tour ' . $tour->tour_name . ' (Trip ID: ' . $tour->trip_id . ')') }}"
                       style="text-decoration: none; font-weight: bold; color: green;">
                        WhatsApp
                    </a></p>
                </td>
            </tr>
        </table>
    </div>

    <div class="box">
        <table style="border: none;">
            <tr>
                <td style="border: none;">
                    <h2>Bank Details</h2>
                    <p><strong>Bank:</strong> HDFC</p>
                    <p><strong>Account:</strong> Arctictern Consultancy Services Pvt Ltd</p>
                    <p><strong>Account No.:</strong> 50200044220791</p>
                    <p><strong>Branch:</strong> Adambakkam</p>
                    <p><strong>IFSC:</strong> HDFC0001858</p>
                </td>
                <td style="text-align: right; border: none;">
                    <p>Click here to <br>Pay Online</p>
                    <a href="https://pages.razorpay.com/travels2020">
                        <img src="{{ public_path('images/Razorpayimage.png') }}" width="100">
                    </a>
                </td>
            </tr>
        </table>
    </div>

    <a href="https://goo.gl/maps/wqthbGH6DsLyGxyx6"><img src="{{ public_path('images/Footer.jpg') }}" width="100%" style="margin-top: 40px;"></a>

</body>
</html>
